...
object HBaseUtil extends HBase with CLogger {
  ...
  private case class RowData(
      rowKey: String,
      dataMap: mutable.HashMap[String, String]
  )
  ...
  /** 数据格式转换
    ...
    *   自定义数据格式
    */
  private[this] def formatToRowTelecomData(
  ...
  /** 数据写入table中
    ...
    * @param originData
    *   originData
    * @param dataFormatFunction
    *   dataFormatFunction 数据格式化函数
    * @return
    *   Boolean
    */
  private def put(
      ...
      qualifiers: Array[String],
      originData: String,
      dataFormatFunction: (Array[String], String) => RowData
  ): Boolean = {
